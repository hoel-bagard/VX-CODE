...
def vis_image(img: np.array, ax: matplotlib.axes.Axes = None, bgr2rgb: bool = True) -> matplotlib.axes.Axes:
...
def vis_vxcode_results(
    ...
    for i in range(len(pred_boxes)):
        ...
        save_name_detection = save_dir_detection.joinpath(f"{save_prefix}_{i}.png")
        ...
        save_name_vis_heatmap = save_dir_vis_heatmap.joinpath(f"{save_prefix}_{i}.png")
        vis_heatmap_i = F.resize(result_eval_heatmap_i[None, ...], size=(img.shape[1], img.shape[2])).squeeze()

        kernel_w = patch_size_w_i + (1 - patch_size_w_i % 2)
        kernel_h = patch_size_h_i + (1 - patch_size_h_i % 2)
        blur = torchvision.transforms.GaussianBlur((kernel_w, kernel_h), sigma=max(kernel_h, kernel_w) / 5)

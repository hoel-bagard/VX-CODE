...
from vx_code.explanations.base import BaseExplainer
from vx_code.utils.reward_functions import compute_similarity_score
from vx_code.utils.utils import (
...
class VXCODE(BaseExplainer):
    def __init__(
        ...
        if self.perturbation_mode == "blur":
            kernel_size = 201
            sigma_x = 200
            self.blur_func = torchvision.transforms.GaussianBlur((kernel_size, kernel_size), sigma=sigma_x)
    ...
    def __call__(
        ...
        (_original_proposal_pred_boxes, _original_proposal_pred_scores, _) = compute_predictions_for_proposals(
            self.model,
            inputs,
        ...
        _target_ids = _original_proposal_pred_scores[:, _target_labels].T == _target_scores_expanded
        target_scores = torch.concat([_original_proposal_pred_scores[target_id] for target_id in _target_ids])
        ...
        for idx, target_instance_num in enumerate(range(num_detected_instances)):
            ...
            num_patches_to_stop_combination_i = self.ratio_to_stop_combination * total_num_patches_i
            ...
            while True:
                ...
                if self.num_patches_per_step > 1 and num_computed_patches < num_patches_to_stop_combination_i:
                ...
                if flag_combination:
                    if self.use_ps:
                        _, target_patch_indices_i = self.get_topk_patches_from_combinations(
                            inputs,
                            canvas,
                            patch_img_i,
                            _patch_indices_i,
                ...
                # Get patch pairs
                topk_values, selected_patch_pairs = self.get_topk_patches_from_combinations(
                    inputs,
                    canvas,
                    patch_img_i,
                ...
                if flag_combination:
                    ...
                    list_alignment_patch_id, list_alignment_rewards = self.get_alignment_patches_from_patch_pair(
                        inputs,
                        canvas,
                        target_patch_pairs,
                        patch_img_i,
                    ...
                    for i in range(len(list_alignment_patch_id)):
                        list_results_patch_id[target_instance_num].append(list_alignment_patch_id[i])
                        list_results_rewards[target_instance_num].append(list_alignment_rewards[i])
                    target_patch_id = list_alignment_patch_id
                else:
                    list_results_patch_id[target_instance_num].append(selected_patch_pairs[0])
                    list_results_rewards[target_instance_num].append(topk_values[0])
                    target_patch_id = selected_patch_pairs

                # Update values
                _patch_indices_i = self.remove_target_patches(_patch_indices_i, target_patch_id)
    ...
    def preprocess(
    ...
    ) -> Tuple[torch.Tensor, torch.Tensor, Tuple[int, int], Tuple[int, int], Tuple[int, int]]:
        img_size_h, img_size_w = img.shape[1], img.shape[2]

        patch_size_h, patch_size_w, box_area_ratio = self.get_patch_size(img_size_h, img_size_w, target_box)
        max_size_h = (
            torch.tensor(img_size_h + (patch_size_h - 1)).div(patch_size_h, rounding_mode="floor") * patch_size_h
        )
        max_size_w = (
            torch.tensor(img_size_w + (patch_size_w - 1)).div(patch_size_w, rounding_mode="floor") * patch_size_w
        )
        padding_size = [
            0,
        ...
        # Make patch images
        patch_img = padding_img.unfold(1, patch_size_h, patch_size_h).unfold(2, patch_size_w, patch_size_w)
        num_patches_h, num_patches_w = (
            patch_img.shape[1],
            patch_img.shape[2],
        )
        patch_indices = torch.Tensor(
            np.array(np.meshgrid(np.arange(num_patches_h), np.arange(num_patches_w))).reshape(2, -1).T
        )
    ...
    def get_topk_patches_from_combinations(
        ...
        if patch_indices.shape[0] >= combinations:
            patch_pairs = [list(combo) for combo in itertools.combinations(patch_indices.numpy(), combinations)]
        else:
            patch_pairs = [
                list(combo) for combo in itertools.combinations(patch_indices.numpy(), patch_indices.shape[0])
            ]
        patch_pairs = torch.from_numpy(np.stack(patch_pairs))

        batch_patch_pairs = [patch_pairs[i : i + self.batch_size] for i in range(0, len(patch_pairs), self.batch_size)]
        list_rewards = list()
        for _patch_pairs in batch_patch_pairs:
            ...
            _rewards, _, _ = self.compute_rewards_for_inputs(list_inputs, target_box, target_score, target_label)
    ...
    def get_alignment_patches_from_patch_pair(
        ...
        for permutation in permutations:
            ...
            for patch_id in permutation:
                ...
                reward, _, _ = self.compute_rewards_for_inputs(_inputs, target_box, target_score, target_label)
    ...
    def remove_target_patches(self, original_patches: torch.Tensor, target_patches: List[torch.Tensor]) -> torch.Tensor:
    ...
    def get_patch_size(self, img_size_h: int, img_size_w: int, box: Boxes) -> Tuple[int, int, float]:
    ...
    def select_target_patch_indices_based_on_area(
        ...
        for i in range(len(patch_indices)):
            ...
            if not (search_x_min <= center_x <= search_x_max and search_y_min <= center_y <= search_y_max):
    ...
    def make_input_img(
        ...
        if mode == "ins":
            return insert_patch_img_on_canvas(start_img, patch_img, patch_id, patch_size, num_patches, padding_len)
        else:
    ...
    def make_eval_heatmap(
        ...
        img_size_h = (num_patches_h - 1) * patch_size[0] + (patch_size_h - padding_len_h)
        img_size_w = (num_patches_w - 1) * patch_size[1] + (patch_size_w - padding_len_w)

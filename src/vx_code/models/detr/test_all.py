...
from vx_code.models.detr.util import box_ops
from vx_code.models.detr.util.misc import nested_tensor_from_tensor_list
...
class Tester(unittest.TestCase):
    ...
    def test_hungarian(self):
        ...
        indices_batched = matcher(
            {"pred_logits": logits.repeat(2, 1, 1), "pred_boxes": boxes.repeat(2, 1, 1)}, targets * 2
        )
        ...
        self.assertEqual(self.indices_torch2python(indices_single), self.indices_torch2python([indices_batched[1]]))
        ...
        indices = matcher(
            {"pred_logits": logits.repeat(2, 1, 1), "pred_boxes": boxes.repeat(2, 1, 1)}, targets + targets_empty
        )
        self.assertEqual(len(indices[1][0]), 0)
        indices = matcher(
            {"pred_logits": logits.repeat(2, 1, 1), "pred_boxes": boxes.repeat(2, 1, 1)}, targets_empty * 2
        )
